    ...
    public static void main( String[] args )
    {
		...
		switch(choice) {
			...
			case 2:
				...
				break;
			case 3:
				Collections.sort(chocolatelist,(a,b)->a.sweetnesslevel-b.sweetnesslevel);
				break;
		}
		System.out.println("Chocolates Present in the Gift are:");
		System.out.printf("\n%-15s %-6s %-6s %12s\n\n","NAME","COST","WEIGHT","SWEETNESS");
		for(int i=0;i<chocolatelist.size();i++){
			System.out.printf("%-15s %3d %8.4f %6d\n\n",chocolatelist.get(i).name,chocolatelist.get(i).cost,chocolatelist.get(i).weight,chocolatelist.get(i).sweetnesslevel);
			...
			if(choice==1) {
				for(int i=0;i<sweetlist.size();i++) {
					...
					if(curr instanceof Candy && curr.cost>=min && curr.cost<=max){
						f=1;
						System.out.printf("%-15s %3d %8.2f %6d\n\n",curr.name,curr.cost,curr.weight,curr.sweetnesslevel);
					}
				}
				...
				for(int i=0;i<sweetlist.size();i++) {
					curr=sweetlist.get(i);
					if(curr instanceof Candy && curr.sweetnesslevel>=min && curr.sweetnesslevel<=max){
						f=1;
						System.out.printf("%-15s %3d %8.4f %6d\n\n",curr.name,curr.cost,curr.weight,curr.sweetnesslevel);
					}
				}
